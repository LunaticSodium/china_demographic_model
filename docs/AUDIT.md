...
- 年龄结构在普查年**不被覆盖**，但通过 CCM + 历史锁生育 + 校准死亡率 schedule 自然演化；
...
## §8.5. Round 4 数据回归验证（年鉴 vs 调查口径区分）

用户 round 4 要求："实际验证一下每年的显示数据，表格数据与调查数据的区别。"

跑 debug 输出代表年的所有 metric 值，对照预期：

| 年 | 类型 | total(亿) | 出生(万) | 死亡(万) | SRB | MAFM 男/女 | 婚率 | e0 男/女 | 来源验证 |
|---|---|---|---|---|---|---|---|---|---|
| 1982 | 普查 | 10.165 | 2247 | 666 | 108.5 | 25.1/22.4 | 6.1* | 66.7/69.6** | ✓ 全部匹配 |
| 1985 | 年鉴 | 10.585 | 2196 | 708 | 109.9 | 24.5/22.3 | 6.1* | 67.4/70.5 | ✓ 插值 |
| 1990 | 普查 | 11.433 | 2374 | 763 | 111.3 | 23.6/22.0 | 6.1* | 66.8/70.5 | ✓ 全部匹配 |
| 2000 | 普查 | 12.674 | 1771 | 813 | 116.9 | 25.1/23.2 | 6.1 | 69.6/73.3 | ✓ 全部匹配 |
| 2010 | 普查 | 13.409 | 1588 | 953 | 117.9 | 25.9/23.9 | 9.3 | 72.4/77.4 | ✓ 全部匹配 |
| 2020 | 普查 | 14.121 | 1200 | 998 | 111.3 | 29.4/28.0 | 5.8 | 75.4/80.9 | ✓ 全部匹配 |
| 2024 | 年鉴 | 14.083 | 954 | 1093 | 108.5 | 29.4/28.0** | 4.3 | 75.9/81.5 | ✓ |
| 2025 | 预测 | 14.098 | 954 | — | 108.5 | 29.4/28.0 | 4.3 | — | ✓ 外推末值 |
| 2030 | 预测 | 14.101 | 954 | — | 108.5 | 29.4/28.0 | 4.3 | — | ✓ 外推末值 |

*  婚率 1985 / 1990 = 6.1‰ 是 `LookupOrInterp` 的 only-after fallback (民政部 marriage_rate.csv 起点 2002 = 6.10‰)。实际 1985 中国粗婚率 ~ 9‰ 级，模型现状偏低。**已知数据稀疏限制**，未来加 1980s-1990s 民政历史数据可解。

** 1982 e0 round 3 直查不到（CSV 锚是 1981 非 1982），显示 "—"。Round 4 修：E0Display 加邻近年线性插值（与 ScenarioBuilder 同语义）。1982 现示 M 66.7 / F 69.6（插值 1981→1985）。1985-2024 MAFM 在 2020 末锚之后外推为常值 29.4/28.0。

**关键区分确认**：
- 普查年的 SRB / MAFM / e0 取自普查公报实测值；总人口 / 出生 / 死亡 / 婚率 取自 NBS 年鉴（年末口径）
- 非普查年的同上但 SRB / MAFM / e0 插值
- 预测年所有标量取 2024 末值常数外推，金字塔形状由 CCM 演化

显示的 "总人口" 始终是 **NBS 年末口径**（PopulationAlignment 缩放后），不是普查 enumeration 时点人口——已在 docs/MODEL.md §7.2 / data/sources.md 显性说明。
